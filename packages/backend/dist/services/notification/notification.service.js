...
class NotificationService {
    ...
    pushAdapter;
    constructor(options) {
        this.repository = options?.repository || new notification_repository_1.NotificationRepository();
        this.socketAdapter = options?.socketAdapter || new types_1.NoOpSocketAdapter();
        this.pushAdapter = options?.pushAdapter || new types_1.NoOpPushAdapter();
    }
    /**
    ...
    async create(data) {
        ...
        if (!data.userId) {
            throw new types_1.NotificationError('User ID is required', 400, {
                message: 'Target user ID must be provided',
            });
        }
        if (!data.eventType) {
            throw new types_1.NotificationError('Event type is required', 400, {
    ...
    async markAsRead(notificationId, userId) {
        if (!notificationId) {
            throw new types_1.NotificationError('Notification ID is required', 400, {
        ...
        if (!notification) {
            throw new types_1.NotificationError('Notification not found', 404, {
     ...
     */
    async deliverOnReconnection(userId) {
        if (!userId) {
            return [];
        }
    ...
    async updatePreferences(userId, preferences) {
        ...
        if (!preferences || preferences.length === 0) {
            throw new types_1.NotificationError('Preferences are required', 400, {
