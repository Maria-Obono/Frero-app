...
class SocialRepository {
     ...
     */
    async blockExistsBetween(userId1, userId2) {
     ...
     * Returns the number of affected rows.
     */
    async deleteFollowsBetween(userId1, userId2) {
        return this.db('follows')
            .where(function () {
            this.where(function () {
     ...
     * Returns the number of affected rows.
     */
    async deleteFriendshipBetween(userId1, userId2) {
     ...
     * Returns the number of affected rows.
     */
    async deletePendingRequestsBetween(userId1, userId2) {
    ...
    async getFriendIds(userId) {
        ...
        return friendships.map((f) => f.user_id_1 === userId ? f.user_id_2 : f.user_id_1);
    }
    /**
     * Count mutual friends between two users.
     * Uses a SQL intersection query for efficiency.
     */
    async countMutualFriends(userId1, userId2) {
     ...
     */
    async getFriendsPaginated(userId, cursor, limit) {
      ...
      `, [userId]))
            ...
            .select('users.id', 'users.username', 'users.display_name', 'users.avatar_url', 'friendships.id as friendship_id');
        if (cursor) {
     ...
     */
    async getFollowersPaginated(userId, cursor, limit) {
        let query = this.db('follows')
            .join('users', 'follows.follower_id', '=', 'users.id')
            .where('follows.followed_id', userId)
            .select('users.id', 'users.username', 'users.display_name', 'users.avatar_url', 'follows.id as follow_id');
        if (cursor) {
            const cursorId = parseInt(cursor, 10);
            if (!isNaN(cursorId)) {
     ...
     */
    async getFollowingPaginated(userId, cursor, limit) {
        let query = this.db('follows')
     ...
     */
    getLastCursorFromFriends(results) {
        if (results.length === 0)
            return null;
