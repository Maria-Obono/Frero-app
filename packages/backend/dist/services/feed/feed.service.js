...
class FeedService {
    ...
    constructor(deps) {
        ...
        this.cacheGet = deps?.cacheGet || redis_utils_1.getCachedFeed;
        this.cacheSet = deps?.cacheSet || redis_utils_1.cacheFeed;
        this.cacheInvalidate = deps?.cacheInvalidate || redis_utils_1.invalidateFeed;
    }
    /**
     ...
     */
    async getPersonalizedFeed(userId, cursor) {
        if (!userId || userId <= 0) {
            throw new types_1.FeedServiceError('Valid userId is required', 400);
        }
        try {
     ...
     */
    async generatePersonalizedFeed(userId, cursor) {
        // Determine pagination offset from cursor
        const cursorOffset = cursor ? parseInt(cursor, 10) : 0;
        if (cursor && isNaN(cursorOffset)) {
            throw new types_1.FeedServiceError('Invalid cursor format', 400);
        }
        // Requirement 9.4: Max 500 posts per session
        if (cursorOffset >= types_1.FEED_MAX_POSTS_PER_SESSION) {
            return {
                data: [],
                cursor: null,
                hasMore: false,
        ...
        sinceDate.setDate(sinceDate.getDate() - types_1.FEED_RECENCY_WINDOW_DAYS);
        const [networkPosts, trendingPosts, interestSignals] = await Promise.all([
            this.repository.getPostsFromAuthors(validNetworkIds, sinceDate, [], // No exclusions for initial fetch; we'll handle via cursor
            types_1.FEED_MAX_POSTS_PER_SESSION),
            this.fetchTrendingPosts([], blockedIds),
            this.repository.getUserInterestSignals(userId, types_1.RECENT_INTERACTION_DAYS),
        ]);
        ...
        const limitedPosts = rankedPosts.slice(0, types_1.FEED_MAX_POSTS_PER_SESSION);
     ...
     */
    rankPosts(posts, interestSignals) {
        if (posts.length === 0)
            return [];
        const now = Date.now();
        const recentInteractionDaysMs = types_1.RECENT_INTERACTION_DAYS * 24 * 60 * 60 * 1000;
        ...
        const scoredPosts = posts.map((post) => {
            const engagementScore = Math.log(1 + (post.like_count || 0) + (post.comment_count || 0) * 2 + (post.share_count || 0) * 3);
            // Recency: exponential decay over 7 days
            const ageMs = now - new Date(post.created_at).getTime();
            const maxAgeMs = types_1.FEED_RECENCY_WINDOW_DAYS * 24 * 60 * 60 * 1000;
            ...
            if (authorWeights.has(post.user_id)) {
                interestBoost = types_1.RECENT_INTERACTION_BOOST;
            }
     ...
     */
    async fetchTrendingPosts(excludePostIds, blockedUserIds) {
        const sinceDate = new Date();
        sinceDate.setDate(sinceDate.getDate() - types_1.FEED_RECENCY_WINDOW_DAYS);
        return this.repository.getTrendingPosts(sinceDate, excludePostIds, blockedUserIds, types_1.FEED_PAGE_SIZE);
    }
    /**
     * Paginate from cached post IDs.
     * Returns the appropriate page of posts from the cached ranked list.
     */
    async paginateFromCache(cachedPostIds, offset) {
        const start = offset;
        const end = Math.min(start + types_1.FEED_PAGE_SIZE, cachedPostIds.length);
        ...
        const hasMore = end < cachedPostIds.length && end < types_1.FEED_MAX_POSTS_PER_SESSION;
     ...
     */
    paginateFromRanked(rankedPosts, offset) {
        const start = offset;
        const end = Math.min(start + types_1.FEED_PAGE_SIZE, rankedPosts.length);
        ...
        const hasMore = end < rankedPosts.length && end < types_1.FEED_MAX_POSTS_PER_SESSION;
        const nextCursor = hasMore ? String(end) : null;
        return {
            data: pagePosts,
            cursor: nextCursor,
     ...
     */
    async getChronologicalFallback(userId, cursor) {
        const cursorOffset = cursor ? parseInt(cursor, 10) : 0;
        if (cursorOffset >= types_1.FEED_MAX_POSTS_PER_SESSION) {
            return {
                data: [],
        ...
        const posts = await this.repository.getChronologicalFeed(validNetworkIds, [], types_1.FEED_MAX_POSTS_PER_SESSION);
        const start = cursorOffset;
        const end = Math.min(start + types_1.FEED_PAGE_SIZE, posts.length);
        const pagePosts = posts.slice(start, end);
        const hasMore = end < posts.length && end < types_1.FEED_MAX_POSTS_PER_SESSION;
        const nextCursor = hasMore ? String(end) : null;
        return {
            data: pagePosts,
            cursor: nextCursor,
